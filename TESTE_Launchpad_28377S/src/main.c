...
void main(void)
{
    ...
    while(1)
    {
        //TESTA_LEDS();                 // NOK - Sem conexão
        //TESTA_BOTOES();               // NOK - Sem conexão
        //TESTA_RS232();                // OK
        //TESTA_RELES();                // NOK - Não tem pino
        //TESTA_DAC_SPI();              // OK
        //TESTA_PWM();                  // OK
        TESTA_BKR_RST();              //
        //TESTA_SAIDAS_ISOLADAS();      // OK - Não tem Saída 3
        //TESTA_ENCODER();              // OK
        //TESTA_ENTRADAS_ANALOGICAS();  // OK - Vy e Vv não tem pinos
    }
}
...
void TESTA_PWM(void)
{
	...
	EPWM1_Regs.TZCLR.bit.OST = 1;
	EPWM1_Regs.TZEINT.bit.OST = 1;

	EPWM2_Regs.TZCLR.bit.OST = 1;
	EPWM2_Regs.TZEINT.bit.OST = 1;

	EPWM3_Regs.TZCLR.bit.OST = 1;
	EPWM3_Regs.TZEINT.bit.OST = 1;

	EPWM4_Regs.TZCLR.bit.OST = 1;
    EPWM4_Regs.TZEINT.bit.OST = 1;

    EPWM5_Regs.TZCLR.bit.OST = 1;
    EPWM5_Regs.TZEINT.bit.OST = 1;

    EPWM6_Regs.TZCLR.bit.OST = 1;
    EPWM6_Regs.TZEINT.bit.OST = 1;
    EDIS;
    ...
    while(cont<20)
    {
        ...
        //#######################################
        //  ATUALIZA PWM 1-3
        EPWM1_Regs.CMPA.bit.CMPA = 2500*(0.5*(1+m));            //EPwm1A
        EPWM1_Regs.CMPB.bit.CMPB = EPWM1_Regs.CMPA.bit.CMPA;	//EPwm1B
        EPWM2_Regs.CMPA.bit.CMPA = 2500*(0.5*(1+m));            //EPwm2A
        EPWM2_Regs.CMPB.bit.CMPB = EPWM2_Regs.CMPA.bit.CMPA;    //EPwm2B
        EPWM3_Regs.CMPA.bit.CMPA = 2500*(0.5*(1+m));            //EPwm3A
        EPWM3_Regs.CMPB.bit.CMPB = EPWM3_Regs.CMPA.bit.CMPA;    //EPwm3B

        //#######################################
        //  ATUALIZA PWM4-6
        EPWM4_Regs.CMPA.bit.CMPA = 2500*(0.5*(1+m));            //EPwm4A
        EPWM4_Regs.CMPB.bit.CMPB = EPWM4_Regs.CMPA.bit.CMPA;    //EPwm4B
        EPWM5_Regs.CMPA.bit.CMPA = 2500*(0.5*(1+m));            //EPwm5A
        EPWM5_Regs.CMPB.bit.CMPB = EPWM5_Regs.CMPA.bit.CMPA;    //EPwm5B
        EPWM6_Regs.CMPA.bit.CMPA = 2500*(0.5*(1+m));            //EPwm6A
        EPWM6_Regs.CMPB.bit.CMPB = EPWM6_Regs.CMPA.bit.CMPA;    //EPwm6B
...
void TESTA_RS232(void)
{
    ...
    while(cont<20)
    {
        if(flag >= 1600000){
            ...
            if (RS232C_buffer == 'T'){
                RS232C_buffer=0;
                sprintf(RS232C_message,"RS232OK!");
                SCIC_TX();
            }
        }
        flag++;
...
// Sensores lado esquerdo da placa
	// Ix, Iy e Iz e Icc2
	Ix   = (AdcaResultRegs.ADCRESULT0);
	Iy   = (AdcbResultRegs.ADCRESULT2);
	Iz   = (AdcbResultRegs.ADCRESULT0);
	Icc2 = (AdcbResultRegs.ADCRESULT4);

	// Vx, Vy e Vz Vcc2
	Vx   = (AdcaResultRegs.ADCRESULT1);
//	Vy   = (AdcaResultRegs.ADCRESULT0);
	Vz   = (AdcbResultRegs.ADCRESULT1);
	Vcc2 = (AdcaResultRegs.ADCRESULT6);

// Sensores lado direito da placa

	// Iu, Iv e Iw e Icc1
	Iu   = (AdcbResultRegs.ADCRESULT6);
//	Iv   = (AdcaResultRegs.ADCRESULT1);
	Iw   = (AdcaResultRegs.ADCRESULT5);
	Icc1 = (AdcaResultRegs.ADCRESULT3);

	// Vu, Vz e Vw Vcc1
	Vu   = (AdcaResultRegs.ADCRESULT2);
	Vv   = (AdcaResultRegs.ADCRESULT4);
	Vw   = (AdcbResultRegs.ADCRESULT3);
	Vcc1 = (AdcbResultRegs.ADCRESULT5);
