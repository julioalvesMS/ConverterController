...
//--------------------------------------------------------------------------------------
   EALLOW;
	...
	// Comment out other unwanted lines.
	   ...
	   GpioCtrlRegs.GPAPUD.bit.GPIO10 = 1;    // Disable pull-up on GPIO10 (EPWM10A)

	/* Configure EPWM-10 pins using GPIO regs*/
	// This specifies which of the possible GPIO pins will be EPWM10 functional pins.
	// Comment out other unwanted lines.

	   GpioCtrlRegs.GPAMUX1.bit.GPIO10 = 1;   // 5=EPWM10A. Configure GPIO18 as EPWM10A
	   GpioCtrlRegs.GPAGMUX1.bit.GPIO10 = 1;   // Configure GPIO18 as EPWM10A
//--------------------------------------------------------------------------------------
//  GPIO-13 - PIN FUNCTION = LED D10 on the F2837xS LaunchPad
//--------------------------------------------------------------------------------------
		GpioCtrlRegs.GPAMUX2.bit.GPIO31 = 0;	// 0=GPIO,  1=Resv,  2=Resv,  3=Resv
...
// Configure the ADCs and power them up
	ADC_SOC_CNF(3, ChSel1,TrigSel1,ACQPS1, 16, 0,  ADC_RESOLUTION_12BIT, ADC_SIGNALMODE_SINGLE);	// Mode= Start/Stop (0)
...
//===========================================================================
	...
	EPwm6Regs.CMPA.bit.CMPA = 1;				// Active load enabled initially
...
//-----------------------------------------
{
	if (Active_LD1_EN == 1)
	{
		if (Continuous_ON)
		{
			EPwm6Regs.CMPA.bit.CMPA = Active_load_PRD;
//			Gui_dlog_trig1 = Gui_Vout1;
		...
		{
		    EPwm6Regs.CMPA.bit.CMPA = Active_load_PRD>>1;
