...
// Note: Assumes GPIO pins for SCIA are configured to Primary function    
	...
	SciaRegs.SCILBAUD.bit.BAUD = (SCI_VBUS_CLOCKRATE/(8*SCI_BAUDRATE))-1;//0x0020;			// 20h = 57.6Kbaud @ LSPCLK = 60/4 MHz
    ...
    SciaRegs.SCIPRI.bit.FREESOFT=0x2;		// Free run
...
void EchoCmdByte(void) // Task 2
{
    ...
    {
    	SciaRegs.SCITXBUF.bit.TXDT =RxChar;			// if yes, echo back the received char
    	CmdPacket[0] = RxChar;
	...
	if(CmdPacket[0] == 0x04 || CmdPacket[0] == 0x06){
		...
		case 1:  //send LSB
			if(wordsLeftToGet > 0)
			{
				...
				{
					SciaRegs.SCITXBUF.bit.TXDT = dataOut16 & 0x000000FF;
					SendTaskPtr = 2;
				}
				else
				{
		...
		case 2: //send MSB
			if (SciaRegs.SCICTL2.bit.TXRDY == 1)
			{
				SciaRegs.SCITXBUF.bit.TXDT = (dataOut16>>8 & 0x000000FF);
	...
	else
	{
		...
		case 1:  //send LSB
			if(wordsLeftToGet > 0)
			{
				if (SciaRegs.SCICTL2.bit.TXRDY == 1)
				{
					SciaRegs.SCITXBUF.bit.TXDT = (dataOut32 & 0x000000FF);
					SendTaskPtr = 2;
				}
				else
				{
		...
		case 2:
			if (SciaRegs.SCICTL2.bit.TXRDY == 1)
			{
				SciaRegs.SCITXBUF.bit.TXDT = (dataOut32>>8 & 0x000000FF);
				SendTaskPtr = 3;
			}
			else
			{
		...
		case 3:
			if(SciaRegs.SCICTL2.bit.TXRDY == 1)
			{
				SciaRegs.SCITXBUF.bit.TXDT = (dataOut32>>16 & 0x000000FF);
				SendTaskPtr = 4;
			}
			else
			{
		...
		case 4:	//send MSB
			if(SciaRegs.SCICTL2.bit.TXRDY == 1)
			{
				SciaRegs.SCITXBUF.bit.TXDT = (dataOut32>>24 & 0x000000FF);
