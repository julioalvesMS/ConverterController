#include <src/Converter/buck.h>

using namespace SwitchedSystem;
using namespace BaseConverter;
using namespace Controller;

extern ControlStrategy controlStrategy;

namespace ConverterBuck
{
    static System system;
    static System discreteSystem;
    static Cycle limitCycle;


    System* Buck::GetSys()
    {
        DefineSystem();

        system.N = 2;

        return &(system);
    }


    System* Buck::GetDiscreteSys()
    {
        DefineDiscreteSystem();

        discreteSystem.N = 2;

        return &(discreteSystem);
    }


    Cycle* Buck::GetLimitCycle()
    {
        switch(controlStrategy)
        {
        case CS_LIMIT_CYCLE_COST:
            DefineLimitCycleCost();
            break;
        case CS_LIMIT_CYCLE_H2:
            DefineLimitCycleH2();
            break;
        case CS_LIMIT_CYCLE_Hinf:
            DefineLimitCycleHinf();
        default:
            break;
        }

        return &(limitCycle);
    }


    void Buck::GetP(double P[SYSTEM_ORDER][SYSTEM_ORDER])
    {
        switch(controlStrategy)
        {
        case CS_CONTINUOUS_THEOREM_1:
            //
            // Buck Converter - Rule 1
            //
            P[0][0] = 0.00021644;
            P[0][1] = 7.95609e-06;
            P[1][0] = 7.95609e-06;
            P[1][1] = 0.00025034;
            break;

        case CS_CONTINUOUS_THEOREM_2:
            //
            // Buck Converter - Rule 2
            //
            P[0][0] = 0.000216438;
            P[0][1] = 7.95555e-06;
            P[1][0] = 7.95555e-06;
            P[1][1] = 0.000250337;
            break;

        case CS_DISCRETE_THEOREM_1:
            //
            // Buck Converter - Discrete Rule 1
            //
            P[0][0] = 4.79776e-06;
            P[0][1] = 1.28986e-06;
            P[1][0] = 1.28986e-06;
            P[1][1] = 9.1124e-06;
            break;

        default:
            break;
        }
    }



    void Buck::GetH(double h[SYSTEM_ORDER])
    {
        switch(controlStrategy)
        {
        case CS_DISCRETE_THEOREM_1:
            //
            // Buck Converter - Discrete Rule 1
            //
            h[0] = 2.40687e-18;
            h[1] = 8.76823e-19;
            break;

        default:
            break;
        }
    }



    double Buck::GetD(double P[SYSTEM_ORDER][SYSTEM_ORDER], double h[SYSTEM_ORDER])
    {
        double d = 0;

        switch(controlStrategy)
        {
        case CS_DISCRETE_THEOREM_1:
            //
            // Buck Converter - Discrete Rule 1
            //
            d = 1.21347e-30;
            break;

        default:
            break;
        }

        return d;
    }



    void Buck::GetClassicVoltageController(double num[2], double den[2])
    {
        num[0] = 0.00676;
        num[1] = -0.0066035;

        den[0] = 1;
        den[1] = -1;
    }



    void Buck::GetClassicVoltageCurrentController(double vNum[2], double vDen[2], double iNum[2], double iDen[2])
    {
        vNum[0] = 0.316;
        vNum[1] = -0.315839;

        vDen[0] = 1;
        vDen[1] = -1;

        iNum[0] = 0.0203;
        iNum[1] = -0.0200615;

        iDen[0] = 1;
        iDen[1] = -1;
    }



    void Buck::GetStateFeedbackH2Controller(double K[2], double C[2], double* M)
    {
        K[0] = 1.21908;
        K[1] = 0.661907;

        C[0] = 1.8;
        C[1] = 1;

        (*M) = 0.677367;
    }



    void Buck::GetReferenceController(double num[2], double den[2])
    {
        num[0] = 1;
        num[1] = -0.9;

        den[0] = 1;
        den[1] = -1;
    }



    int Buck::SubSystem2SwitchState(int SubSystem)
    {
        return SubSystem;
    }


    void DefineSystem()
    {
        SubSystem* subSys;

        //
        // =============== Subsystem 1 ===============
        //
        subSys = &(system.subSystems[0]);

        //
        // Subsystem 1 -- Matrix A
        //
        subSys->A[0][0] = -R/L;
        subSys->A[0][1] = -1/L;
        subSys->A[1][0] =  1/Co;
        subSys->A[1][1] = -1/(Ro*Co);
        //
        // Subsystem 1 -- Matrix B
        //
        subSys->B[0] = 1/L;
        subSys->B[1] = 0;
        //
        // Subsystem 1 -- Matrix Q
        //
        subSys->Q[0][0] = 0;
        subSys->Q[0][1] = 0;
        subSys->Q[1][0] = 0;
        subSys->Q[1][1] = 1/Ro;


        //
        // =============== Subsystem 2 ===============
        //
        subSys = &(system.subSystems[1]);

        //
        // Subsystem 2 -- Matrix A
        //
        subSys->A[0][0] = -R/L;
        subSys->A[0][1] = -1/L;
        subSys->A[1][0] =  1/Co;
        subSys->A[1][1] = -1/(Ro*Co);
        //
        // Subsystem 2 -- Matrix B
        //
        subSys->B[0] = 0;
        subSys->B[1] = 0;
        //
        // Subsystem 2 -- Matrix Q
        //
        subSys->Q[0][0] = 0;
        subSys->Q[0][1] = 0;
        subSys->Q[1][0] = 0;
        subSys->Q[1][1] = 1/Ro;
    }


    void DefineDiscreteSystem()
    {
        SubSystem* subSys;

        //
        // =============== Subsystem 1 ===============
        //
        subSys = &(discreteSystem.subSystems[0]);

        //
        // Subsystem 1 -- Matrix A
        //
        subSys->A[0][0] = 0.993766;
        subSys->A[0][1] = -0.0125799;
        subSys->A[1][0] = 0.0110759;
        subSys->A[1][1] = 0.999815;
        //
        // Subsystem 1 -- Matrix L
        //
        subSys->L[0] = 0.00674481;
        subSys->L[1] = 3.75088e-05;
        //
        // Subsystem 1 -- Matrix Q
        //
        subSys->E[0][0] = 0;
        subSys->E[0][1] = 0;
        subSys->E[1][0] = 0;
        subSys->E[1][1] = 0.00103306;
        //
        // Subsystem 1 -- Matrix H
        //
        subSys->H[0] = 0;
        subSys->H[1] = 0;
        //
        // Subsystem 1 -- Matrix G
        //
        subSys->G[0] = 0;
        subSys->G[1] = 0;
        //
        // Subsystem 1 -- Matrix Q
        //
        subSys->Q[0][0] = 0.1;
        subSys->Q[0][1] = 0;
        subSys->Q[1][0] = 0;
        subSys->Q[1][1] = 0.0103306;


        //
        // =============== Subsystem 2 ===============
        //
        subSys = &(discreteSystem.subSystems[1]);

        //
        // Subsystem 2 -- Matrix A
        //
        subSys->A[0][0] = 0.993766;
        subSys->A[0][1] = -0.0125799;
        subSys->A[1][0] = 0.0110759;
        subSys->A[1][1] = 0.999815;
        //
        // Subsystem 2 -- Matrix L
        //
        subSys->L[0] = -0.00583585;
        subSys->L[1] = -3.24539e-05;
        //
        // Subsystem 2 -- Matrix Q
        //
        subSys->E[0][0] = 0;
        subSys->E[0][1] = 0;
        subSys->E[1][0] = 0;
        subSys->E[1][1] = 0.00103306;
        //
        // Subsystem 2 -- Matrix H
        //
        subSys->H[0] = 0;
        subSys->H[1] = 0;
        //
        // Subsystem 2 -- Matrix G
        //
        subSys->G[0] = 0;
        subSys->G[1] = 0;
        //
        // Subsystem 2 -- Matrix Q
        //
        subSys->Q[0][0] = 0.1;
        subSys->Q[0][1] = 0;
        subSys->Q[1][0] = 0;
        subSys->Q[1][1] = 0.0103306;

    }



    void DefineLimitCycleCost()
    {
        CycleStep* step;

        limitCycle.kappa = 13;

        //
        // =============== Limit Cycle Step 1 ===============
        //
        step = &(limitCycle.cycleSteps[0]);

        //
        // Cycle Step 1 -- Matrix P
        //
        step->P[0][0] = 8.70775;
        step->P[0][1] = 0.318351;
        step->P[1][0] = 0.318351;
        step->P[1][1] = 10.0188;
        //
        // Cycle Step 1 -- Vector Xe
        //
        step->Xe[0] = -0.567224;
        step->Xe[1] = 29.8339;
        //
        // Cycle Step 1 -- Matrix ell
        //
        step->ell[0][0] = -1.11022e-16;
        step->ell[0][1] = 7.09328e-15;
        step->ell[1][0] = -0.817743;
        step->ell[1][1] = -0.00454758;

        //
        // =============== Limit Cycle Step 2 ===============
        //
        step = &(limitCycle.cycleSteps[1]);

        //
        // Cycle Step 2 -- Matrix P
        //
        step->P[0][0] = 8.70775;
        step->P[0][1] = 0.318351;
        step->P[1][0] = 0.318351;
        step->P[1][1] = 10.0188;
        //
        // Cycle Step 2 -- Vector Xe
        //
        step->Xe[0] = -0.121254;
        step->Xe[1] = 29.8267;
        //
        // Cycle Step 2 -- Matrix ell
        //
        step->ell[0][0] = -3.33067e-16;
        step->ell[0][1] = -3.56486e-15;
        step->ell[1][0] = -0.817743;
        step->ell[1][1] = -0.00454758;

        //
        // =============== Limit Cycle Step 3 ===============
        //
        step = &(limitCycle.cycleSteps[2]);

        //
        // Cycle Step 3 -- Matrix P
        //
        step->P[0][0] = 8.70775;
        step->P[0][1] = 0.318351;
        step->P[1][0] = 0.318351;
        step->P[1][1] = 10.0188;
        //
        // Cycle Step 3 -- Vector Xe
        //
        step->Xe[0] = 0.322028;
        step->Xe[1] = 29.8244;
        //
        // Cycle Step 3 -- Matrix ell
        //
        step->ell[0][0] = -1.11022e-16;
        step->ell[0][1] = -1.21431e-17;
        step->ell[1][0] = -0.817743;
        step->ell[1][1] = -0.00454758;

        //
        // =============== Limit Cycle Step 4 ===============
        //
        step = &(limitCycle.cycleSteps[3]);

        //
        // Cycle Step 4 -- Matrix P
        //
        step->P[0][0] = 8.70775;
        step->P[0][1] = 0.318351;
        step->P[1][0] = 0.318351;
        step->P[1][1] = 10.0188;
        //
        // Cycle Step 4 -- Vector Xe
        //
        step->Xe[0] = 0.762574;
        step->Xe[1] = 29.827;
        //
        // Cycle Step 4 -- Matrix ell
        //
        step->ell[0][0] = -5.55112e-16;
        step->ell[0][1] = -1.21431e-17;
        step->ell[1][0] = -0.817743;
        step->ell[1][1] = -0.00454758;

        //
        // =============== Limit Cycle Step 5 ===============
        //
        step = &(limitCycle.cycleSteps[4]);

        //
        // Cycle Step 5 -- Matrix P
        //
        step->P[0][0] = 8.70775;
        step->P[0][1] = 0.318351;
        step->P[1][0] = 0.318351;
        step->P[1][1] = 10.0188;
        //
        // Cycle Step 5 -- Vector Xe
        //
        step->Xe[0] = 1.20034;
        step->Xe[1] = 29.8344;
        //
        // Cycle Step 5 -- Matrix ell
        //
        step->ell[0][0] = 4.44089e-16;
        step->ell[0][1] = -1.21431e-17;
        step->ell[1][0] = -0.817743;
        step->ell[1][1] = -0.00454758;

        //
        // =============== Limit Cycle Step 6 ===============
        //
        step = &(limitCycle.cycleSteps[5]);

        //
        // Cycle Step 6 -- Matrix P
        //
        step->P[0][0] = 8.70775;
        step->P[0][1] = 0.318351;
        step->P[1][0] = 0.318351;
        step->P[1][1] = 10.0188;
        //
        // Cycle Step 6 -- Vector Xe
        //
        step->Xe[0] = 1.63529;
        step->Xe[1] = 29.8468;
        //
        // Cycle Step 6 -- Matrix ell
        //
        step->ell[0][0] = 0.817743;
        step->ell[0][1] = 0.00454758;
        step->ell[1][0] = 2.22045e-16;
        step->ell[1][1] = 0;

        //
        // =============== Limit Cycle Step 7 ===============
        //
        step = &(limitCycle.cycleSteps[6]);

        //
        // Cycle Step 7 -- Matrix P
        //
        step->P[0][0] = 8.70775;
        step->P[0][1] = 0.318351;
        step->P[1][0] = 0.318351;
        step->P[1][1] = 10.0188;
        //
        // Cycle Step 7 -- Vector Xe
        //
        step->Xe[0] = 1.24962;
        step->Xe[1] = 29.8594;
        //
        // Cycle Step 7 -- Matrix ell
        //
        step->ell[0][0] = 0.817743;
        step->ell[0][1] = 0.00454758;
        step->ell[1][0] = -3.33067e-16;
        step->ell[1][1] = 0;

        //
        // =============== Limit Cycle Step 8 ===============
        //
        step = &(limitCycle.cycleSteps[7]);

        //
        // Cycle Step 8 -- Matrix P
        //
        step->P[0][0] = 8.70775;
        step->P[0][1] = 0.318351;
        step->P[1][0] = 0.318351;
        step->P[1][1] = 10.0188;
        //
        // Cycle Step 8 -- Vector Xe
        //
        step->Xe[0] = 0.8662;
        step->Xe[1] = 29.8677;
        //
        // Cycle Step 8 -- Matrix ell
        //
        step->ell[0][0] = 0.817743;
        step->ell[0][1] = 0.00454758;
        step->ell[1][0] = 5.55112e-17;
        step->ell[1][1] = 3.55271e-15;

        //
        // =============== Limit Cycle Step 9 ===============
        //
        step = &(limitCycle.cycleSteps[8]);

        //
        // Cycle Step 9 -- Matrix P
        //
        step->P[0][0] = 8.70775;
        step->P[0][1] = 0.318351;
        step->P[1][0] = 0.318351;
        step->P[1][1] = 10.0188;
        //
        // Cycle Step 9 -- Vector Xe
        //
        step->Xe[0] = 0.485066;
        step->Xe[1] = 29.8718;
        //
        // Cycle Step 9 -- Matrix ell
        //
        step->ell[0][0] = 0.817743;
        step->ell[0][1] = 0.00454758;
        step->ell[1][0] = -4.30211e-16;
        step->ell[1][1] = 0;

        //
        // =============== Limit Cycle Step 10 ===============
        //
        step = &(limitCycle.cycleSteps[9]);

        //
        // Cycle Step 10 -- Matrix P
        //
        step->P[0][0] = 8.70775;
        step->P[0][1] = 0.318351;
        step->P[1][0] = 0.318351;
        step->P[1][1] = 10.0188;
        //
        // Cycle Step 10 -- Vector Xe
        //
        step->Xe[0] = 0.106257;
        step->Xe[1] = 29.8716;
        //
        // Cycle Step 10 -- Matrix ell
        //
        step->ell[0][0] = 0.817743;
        step->ell[0][1] = 0.00454758;
        step->ell[1][0] = 4.996e-16;
        step->ell[1][1] = 0;

        //
        // =============== Limit Cycle Step 11 ===============
        //
        step = &(limitCycle.cycleSteps[10]);

        //
        // Cycle Step 11 -- Matrix P
        //
        step->P[0][0] = 8.70775;
        step->P[0][1] = 0.318351;
        step->P[1][0] = 0.318351;
        step->P[1][1] = 10.0188;
        //
        // Cycle Step 11 -- Vector Xe
        //
        step->Xe[0] = -0.270189;
        step->Xe[1] = 29.8673;
        //
        // Cycle Step 11 -- Matrix ell
        //
        step->ell[0][0] = 0.817743;
        step->ell[0][1] = 0.00454758;
        step->ell[1][0] = 0;
        step->ell[1][1] = 0;

        //
        // =============== Limit Cycle Step 12 ===============
        //
        step = &(limitCycle.cycleSteps[11]);

        //
        // Cycle Step 12 -- Matrix P
        //
        step->P[0][0] = 8.70775;
        step->P[0][1] = 0.318351;
        step->P[1][0] = 0.318351;
        step->P[1][1] = 10.0188;
        //
        // Cycle Step 12 -- Vector Xe
        //
        step->Xe[0] = -0.644233;
        step->Xe[1] = 29.8588;
        //
        // Cycle Step 12 -- Matrix ell
        //
        step->ell[0][0] = 0.817743;
        step->ell[0][1] = 0.00454758;
        step->ell[1][0] = -2.22045e-16;
        step->ell[1][1] = 0;

        //
        // =============== Limit Cycle Step 13 ===============
        //
        step = &(limitCycle.cycleSteps[12]);

        //
        // Cycle Step 13 -- Matrix P
        //
        step->P[0][0] = 8.70775;
        step->P[0][1] = 0.318351;
        step->P[1][0] = 0.318351;
        step->P[1][1] = 10.0188;
        //
        // Cycle Step 13 -- Vector Xe
        //
        step->Xe[0] = -1.01584;
        step->Xe[1] = 29.8461;
        //
        // Cycle Step 13 -- Matrix ell
        //
        step->ell[0][0] = 0;
        step->ell[0][1] = -3.56486e-15;
        step->ell[1][0] = -0.817743;
        step->ell[1][1] = -0.00454758;

    }



    void DefineLimitCycleH2()
    {
        CycleStep* step;

        limitCycle.kappa = 13;

        //
        // =============== Limit Cycle Step 1 ===============
        //
        step = &(limitCycle.cycleSteps[0]);

        //
        // Cycle Step 1 -- Matrix P
        //
        step->P[0][0] = 6.76101e+15;
        step->P[0][1] = 1.63964e+15;
        step->P[1][0] = 1.63964e+15;
        step->P[1][1] = 9.09836e+15;
        //
        // Cycle Step 1 -- Vector Xe
        //
        step->Xe[0] = -0.00104806;
        step->Xe[1] = 29.8381;
        //
        // Cycle Step 1 -- Matrix ell
        //
        step->ell[0][0] = 0;
        step->ell[0][1] = 3.54057e-15;
        step->ell[1][0] = -0.817743;
        step->ell[1][1] = -0.00454758;

        //
        // =============== Limit Cycle Step 2 ===============
        //
        step = &(limitCycle.cycleSteps[1]);

        //
        // Cycle Step 2 -- Matrix P
        //
        step->P[0][0] = 6.76115e+15;
        step->P[0][1] = 1.63949e+15;
        step->P[1][0] = 1.63949e+15;
        step->P[1][1] = 9.09146e+15;
        //
        // Cycle Step 2 -- Vector Xe
        //
        step->Xe[0] = 0.44134;
        step->Xe[1] = 29.8371;
        //
        // Cycle Step 2 -- Matrix ell
        //
        step->ell[0][0] = 0;
        step->ell[0][1] = -3.56486e-15;
        step->ell[1][0] = -0.817743;
        step->ell[1][1] = -0.00454758;

        //
        // =============== Limit Cycle Step 3 ===============
        //
        step = &(limitCycle.cycleSteps[2]);

        //
        // Cycle Step 3 -- Matrix P
        //
        step->P[0][0] = 6.76121e+15;
        step->P[0][1] = 1.63946e+15;
        step->P[1][0] = 1.63946e+15;
        step->P[1][1] = 9.0857e+15;
        //
        // Cycle Step 3 -- Vector Xe
        //
        step->Xe[0] = 0.880983;
        step->Xe[1] = 29.841;
        //
        // Cycle Step 3 -- Matrix ell
        //
        step->ell[0][0] = -4.44089e-16;
        step->ell[0][1] = -1.21431e-17;
        step->ell[1][0] = -0.817743;
        step->ell[1][1] = -0.00454758;

        //
        // =============== Limit Cycle Step 4 ===============
        //
        step = &(limitCycle.cycleSteps[3]);

        //
        // Cycle Step 4 -- Matrix P
        //
        step->P[0][0] = 6.7612e+15;
        step->P[0][1] = 1.63953e+15;
        step->P[1][0] = 1.63953e+15;
        step->P[1][1] = 9.0811e+15;
        //
        // Cycle Step 4 -- Vector Xe
        //
        step->Xe[0] = 1.31784;
        step->Xe[1] = 29.8498;
        //
        // Cycle Step 4 -- Matrix ell
        //
        step->ell[0][0] = 0.817743;
        step->ell[0][1] = 0.00454758;
        step->ell[1][0] = 1.11022e-16;
        step->ell[1][1] = -3.55271e-15;

        //
        // =============== Limit Cycle Step 5 ===============
        //
        step = &(limitCycle.cycleSteps[4]);

        //
        // Cycle Step 5 -- Matrix P
        //
        step->P[0][0] = 6.76113e+15;
        step->P[0][1] = 1.63966e+15;
        step->P[1][0] = 1.63966e+15;
        step->P[1][1] = 9.07765e+15;
        //
        // Cycle Step 5 -- Vector Xe
        //
        step->Xe[0] = 0.934111;
        step->Xe[1] = 29.8589;
        //
        // Cycle Step 5 -- Matrix ell
        //
        step->ell[0][0] = 0.817743;
        step->ell[0][1] = 0.00454758;
        step->ell[1][0] = -1.11022e-16;
        step->ell[1][1] = 0;

        //
        // =============== Limit Cycle Step 6 ===============
        //
        step = &(limitCycle.cycleSteps[5]);

        //
        // Cycle Step 6 -- Matrix P
        //
        step->P[0][0] = 6.76102e+15;
        step->P[0][1] = 1.63983e+15;
        step->P[1][0] = 1.63983e+15;
        step->P[1][1] = 9.07536e+15;
        //
        // Cycle Step 6 -- Vector Xe
        //
        step->Xe[0] = 0.552664;
        step->Xe[1] = 29.8637;
        //
        // Cycle Step 6 -- Matrix ell
        //
        step->ell[0][0] = 0.817743;
        step->ell[0][1] = 0.00454758;
        step->ell[1][0] = -2.77556e-17;
        step->ell[1][1] = 0;

        //
        // =============== Limit Cycle Step 7 ===============
        //
        step = &(limitCycle.cycleSteps[6]);

        //
        // Cycle Step 7 -- Matrix P
        //
        step->P[0][0] = 6.76088e+15;
        step->P[0][1] = 1.64002e+15;
        step->P[1][0] = 1.64002e+15;
        step->P[1][1] = 9.07423e+15;
        //
        // Cycle Step 7 -- Vector Xe
        //
        step->Xe[0] = 0.173535;
        step->Xe[1] = 29.8643;
        //
        // Cycle Step 7 -- Matrix ell
        //
        step->ell[0][0] = 0.817743;
        step->ell[0][1] = 0.00454758;
        step->ell[1][0] = 1.94289e-16;
        step->ell[1][1] = 0;

        //
        // =============== Limit Cycle Step 8 ===============
        //
        step = &(limitCycle.cycleSteps[7]);

        //
        // Cycle Step 8 -- Matrix P
        //
        step->P[0][0] = 6.76074e+15;
        step->P[0][1] = 1.64019e+15;
        step->P[1][0] = 1.64019e+15;
        step->P[1][1] = 9.07424e+15;
        //
        // Cycle Step 8 -- Vector Xe
        //
        step->Xe[0] = -0.203238;
        step->Xe[1] = 29.8607;
        //
        // Cycle Step 8 -- Matrix ell
        //
        step->ell[0][0] = 0.817743;
        step->ell[0][1] = 0.00454758;
        step->ell[1][0] = -2.22045e-16;
        step->ell[1][1] = 0;

        //
        // =============== Limit Cycle Step 9 ===============
        //
        step = &(limitCycle.cycleSteps[8]);

        //
        // Cycle Step 9 -- Matrix P
        //
        step->P[0][0] = 6.76061e+15;
        step->P[0][1] = 1.64033e+15;
        step->P[1][0] = 1.64033e+15;
        step->P[1][1] = 9.07541e+15;
        //
        // Cycle Step 9 -- Vector Xe
        //
        step->Xe[0] = -0.577617;
        step->Xe[1] = 29.853;
        //
        // Cycle Step 9 -- Matrix ell
        //
        step->ell[0][0] = 0;
        step->ell[0][1] = 3.54057e-15;
        step->ell[1][0] = -0.817743;
        step->ell[1][1] = -0.00454758;

        //
        // =============== Limit Cycle Step 10 ===============
        //
        step = &(limitCycle.cycleSteps[9]);

        //
        // Cycle Step 10 -- Matrix P
        //
        step->P[0][0] = 6.76051e+15;
        step->P[0][1] = 1.6404e+15;
        step->P[1][0] = 1.6404e+15;
        step->P[1][1] = 9.07773e+15;
        //
        // Cycle Step 10 -- Vector Xe
        //
        step->Xe[0] = -0.131821;
        step->Xe[1] = 29.8456;
        //
        // Cycle Step 10 -- Matrix ell
        //
        step->ell[0][0] = 3.33067e-16;
        step->ell[0][1] = -1.21431e-17;
        step->ell[1][0] = -0.817743;
        step->ell[1][1] = -0.00454758;

        //
        // =============== Limit Cycle Step 11 ===============
        //
        step = &(limitCycle.cycleSteps[10]);

        //
        // Cycle Step 11 -- Matrix P
        //
        step->P[0][0] = 6.76048e+15;
        step->P[0][1] = 1.6404e+15;
        step->P[1][0] = 1.6404e+15;
        step->P[1][1] = 9.08119e+15;
        //
        // Cycle Step 11 -- Vector Xe
        //
        step->Xe[0] = 0.311288;
        step->Xe[1] = 29.8432;
        //
        // Cycle Step 11 -- Matrix ell
        //
        step->ell[0][0] = 0.817743;
        step->ell[0][1] = 0.00454758;
        step->ell[1][0] = 4.30211e-16;
        step->ell[1][1] = 0;

        //
        // =============== Limit Cycle Step 12 ===============
        //
        step = &(limitCycle.cycleSteps[11]);

        //
        // Cycle Step 12 -- Matrix P
        //
        step->P[0][0] = 6.76053e+15;
        step->P[0][1] = 1.64028e+15;
        step->P[1][0] = 1.64028e+15;
        step->P[1][1] = 9.08578e+15;
        //
        // Cycle Step 12 -- Vector Xe
        //
        step->Xe[0] = -0.0660776;
        step->Xe[1] = 29.8411;
        //
        // Cycle Step 12 -- Matrix ell
        //
        step->ell[0][0] = 3.33067e-16;
        step->ell[0][1] = -1.21431e-17;
        step->ell[1][0] = -0.817743;
        step->ell[1][1] = -0.00454758;

        //
        // =============== Limit Cycle Step 13 ===============
        //
        step = &(limitCycle.cycleSteps[12]);

        //
        // Cycle Step 13 -- Matrix P
        //
        step->P[0][0] = 6.7607e+15;
        step->P[0][1] = 1.64003e+15;
        step->P[1][0] = 1.64003e+15;
        step->P[1][1] = 9.09151e+15;
        //
        // Cycle Step 13 -- Vector Xe
        //
        step->Xe[0] = 0.376678;
        step->Xe[1] = 29.8394;
        //
        // Cycle Step 13 -- Matrix ell
        //
        step->ell[0][0] = 0.817743;
        step->ell[0][1] = 0.00454758;
        step->ell[1][0] = 2.62377e-17;
        step->ell[1][1] = 0;

    }



    void DefineLimitCycleHinf()
    {
        CycleStep* step;

        limitCycle.kappa = 13;
        limitCycle.rho = 1.78253e-22;

        //
        // =============== Limit Cycle Step 1 ===============
        //
        step = &(limitCycle.cycleSteps[0]);

        //
        // Cycle Step 1 -- Matrix P
        //
        step->P[0][0] = 5.10191e+11;
        step->P[0][1] = 1.21211e+11;
        step->P[1][0] = 1.21211e+11;
        step->P[1][1] = 5.70447e+11;
        //
        // Cycle Step 1 -- Vector Xe
        //
        step->Xe[0] = 1.63529;
        step->Xe[1] = 29.8468;
        //
        // Cycle Step 1 -- Matrix ell
        //
        step->ell[0][0] = 0.817743;
        step->ell[0][1] = 0.00454758;
        step->ell[1][0] = 2.22045e-16;
        step->ell[1][1] = 3.55271e-15;

        //
        // =============== Limit Cycle Step 2 ===============
        //
        step = &(limitCycle.cycleSteps[1]);

        //
        // Cycle Step 2 -- Matrix P
        //
        step->P[0][0] = 5.10191e+11;
        step->P[0][1] = 1.21211e+11;
        step->P[1][0] = 1.21211e+11;
        step->P[1][1] = 5.70447e+11;
        //
        // Cycle Step 2 -- Vector Xe
        //
        step->Xe[0] = 1.24962;
        step->Xe[1] = 29.8594;
        //
        // Cycle Step 2 -- Matrix ell
        //
        step->ell[0][0] = 0.817743;
        step->ell[0][1] = 0.00454758;
        step->ell[1][0] = 1.11022e-16;
        step->ell[1][1] = -3.55271e-15;

        //
        // =============== Limit Cycle Step 3 ===============
        //
        step = &(limitCycle.cycleSteps[2]);

        //
        // Cycle Step 3 -- Matrix P
        //
        step->P[0][0] = 5.10191e+11;
        step->P[0][1] = 1.21211e+11;
        step->P[1][0] = 1.21211e+11;
        step->P[1][1] = 5.70447e+11;
        //
        // Cycle Step 3 -- Vector Xe
        //
        step->Xe[0] = 0.8662;
        step->Xe[1] = 29.8677;
        //
        // Cycle Step 3 -- Matrix ell
        //
        step->ell[0][0] = 0.817743;
        step->ell[0][1] = 0.00454758;
        step->ell[1][0] = -1.66533e-16;
        step->ell[1][1] = 0;

        //
        // =============== Limit Cycle Step 4 ===============
        //
        step = &(limitCycle.cycleSteps[3]);

        //
        // Cycle Step 4 -- Matrix P
        //
        step->P[0][0] = 5.10191e+11;
        step->P[0][1] = 1.21211e+11;
        step->P[1][0] = 1.21211e+11;
        step->P[1][1] = 5.70447e+11;
        //
        // Cycle Step 4 -- Vector Xe
        //
        step->Xe[0] = 0.485066;
        step->Xe[1] = 29.8718;
        //
        // Cycle Step 4 -- Matrix ell
        //
        step->ell[0][0] = 0.817743;
        step->ell[0][1] = 0.00454758;
        step->ell[1][0] = -6.93889e-17;
        step->ell[1][1] = -3.55271e-15;

        //
        // =============== Limit Cycle Step 5 ===============
        //
        step = &(limitCycle.cycleSteps[4]);

        //
        // Cycle Step 5 -- Matrix P
        //
        step->P[0][0] = 5.10191e+11;
        step->P[0][1] = 1.21211e+11;
        step->P[1][0] = 1.21211e+11;
        step->P[1][1] = 5.70447e+11;
        //
        // Cycle Step 5 -- Vector Xe
        //
        step->Xe[0] = 0.106257;
        step->Xe[1] = 29.8716;
        //
        // Cycle Step 5 -- Matrix ell
        //
        step->ell[0][0] = 0.817743;
        step->ell[0][1] = 0.00454758;
        step->ell[1][0] = 1.66533e-16;
        step->ell[1][1] = 3.55271e-15;

        //
        // =============== Limit Cycle Step 6 ===============
        //
        step = &(limitCycle.cycleSteps[5]);

        //
        // Cycle Step 6 -- Matrix P
        //
        step->P[0][0] = 5.10191e+11;
        step->P[0][1] = 1.21211e+11;
        step->P[1][0] = 1.21211e+11;
        step->P[1][1] = 5.70447e+11;
        //
        // Cycle Step 6 -- Vector Xe
        //
        step->Xe[0] = -0.270189;
        step->Xe[1] = 29.8673;
        //
        // Cycle Step 6 -- Matrix ell
        //
        step->ell[0][0] = 0.817743;
        step->ell[0][1] = 0.00454758;
        step->ell[1][0] = 2.22045e-16;
        step->ell[1][1] = 0;

        //
        // =============== Limit Cycle Step 7 ===============
        //
        step = &(limitCycle.cycleSteps[6]);

        //
        // Cycle Step 7 -- Matrix P
        //
        step->P[0][0] = 5.10191e+11;
        step->P[0][1] = 1.21211e+11;
        step->P[1][0] = 1.21211e+11;
        step->P[1][1] = 5.70447e+11;
        //
        // Cycle Step 7 -- Vector Xe
        //
        step->Xe[0] = -0.644233;
        step->Xe[1] = 29.8588;
        //
        // Cycle Step 7 -- Matrix ell
        //
        step->ell[0][0] = 0.817743;
        step->ell[0][1] = 0.00454758;
        step->ell[1][0] = 0;
        step->ell[1][1] = -3.55271e-15;

        //
        // =============== Limit Cycle Step 8 ===============
        //
        step = &(limitCycle.cycleSteps[7]);

        //
        // Cycle Step 8 -- Matrix P
        //
        step->P[0][0] = 5.10191e+11;
        step->P[0][1] = 1.21211e+11;
        step->P[1][0] = 1.21211e+11;
        step->P[1][1] = 5.70447e+11;
        //
        // Cycle Step 8 -- Vector Xe
        //
        step->Xe[0] = -1.01584;
        step->Xe[1] = 29.8461;
        //
        // Cycle Step 8 -- Matrix ell
        //
        step->ell[0][0] = -1.11022e-16;
        step->ell[0][1] = 3.54057e-15;
        step->ell[1][0] = -0.817743;
        step->ell[1][1] = -0.00454758;

        //
        // =============== Limit Cycle Step 9 ===============
        //
        step = &(limitCycle.cycleSteps[8]);

        //
        // Cycle Step 9 -- Matrix P
        //
        step->P[0][0] = 5.10191e+11;
        step->P[0][1] = 1.21211e+11;
        step->P[1][0] = 1.21211e+11;
        step->P[1][1] = 5.70447e+11;
        //
        // Cycle Step 9 -- Vector Xe
        //
        step->Xe[0] = -0.567224;
        step->Xe[1] = 29.8339;
        //
        // Cycle Step 9 -- Matrix ell
        //
        step->ell[0][0] = 2.22045e-16;
        step->ell[0][1] = -1.21431e-17;
        step->ell[1][0] = -0.817743;
        step->ell[1][1] = -0.00454758;

        //
        // =============== Limit Cycle Step 10 ===============
        //
        step = &(limitCycle.cycleSteps[9]);

        //
        // Cycle Step 10 -- Matrix P
        //
        step->P[0][0] = 5.10191e+11;
        step->P[0][1] = 1.21211e+11;
        step->P[1][0] = 1.21211e+11;
        step->P[1][1] = 5.70447e+11;
        //
        // Cycle Step 10 -- Vector Xe
        //
        step->Xe[0] = -0.121254;
        step->Xe[1] = 29.8267;
        //
        // Cycle Step 10 -- Matrix ell
        //
        step->ell[0][0] = 3.33067e-16;
        step->ell[0][1] = -1.21431e-17;
        step->ell[1][0] = -0.817743;
        step->ell[1][1] = -0.00454758;

        //
        // =============== Limit Cycle Step 11 ===============
        //
        step = &(limitCycle.cycleSteps[10]);

        //
        // Cycle Step 11 -- Matrix P
        //
        step->P[0][0] = 5.10191e+11;
        step->P[0][1] = 1.21211e+11;
        step->P[1][0] = 1.21211e+11;
        step->P[1][1] = 5.70447e+11;
        //
        // Cycle Step 11 -- Vector Xe
        //
        step->Xe[0] = 0.322028;
        step->Xe[1] = 29.8244;
        //
        // Cycle Step 11 -- Matrix ell
        //
        step->ell[0][0] = 6.66134e-16;
        step->ell[0][1] = 3.54057e-15;
        step->ell[1][0] = -0.817743;
        step->ell[1][1] = -0.00454758;

        //
        // =============== Limit Cycle Step 12 ===============
        //
        step = &(limitCycle.cycleSteps[11]);

        //
        // Cycle Step 12 -- Matrix P
        //
        step->P[0][0] = 5.10191e+11;
        step->P[0][1] = 1.21211e+11;
        step->P[1][0] = 1.21211e+11;
        step->P[1][1] = 5.70447e+11;
        //
        // Cycle Step 12 -- Vector Xe
        //
        step->Xe[0] = 0.762574;
        step->Xe[1] = 29.827;
        //
        // Cycle Step 12 -- Matrix ell
        //
        step->ell[0][0] = 1.11022e-16;
        step->ell[0][1] = -1.21431e-17;
        step->ell[1][0] = -0.817743;
        step->ell[1][1] = -0.00454758;

        //
        // =============== Limit Cycle Step 13 ===============
        //
        step = &(limitCycle.cycleSteps[12]);

        //
        // Cycle Step 13 -- Matrix P
        //
        step->P[0][0] = 5.10191e+11;
        step->P[0][1] = 1.21211e+11;
        step->P[1][0] = 1.21211e+11;
        step->P[1][1] = 5.70447e+11;
        //
        // Cycle Step 13 -- Vector Xe
        //
        step->Xe[0] = 1.20034;
        step->Xe[1] = 29.8344;
        //
        // Cycle Step 13 -- Matrix ell
        //
        step->ell[0][0] = 0;
        step->ell[0][1] = -1.21431e-17;
        step->ell[1][0] = -0.817743;
        step->ell[1][1] = -0.00454758;

    }

}
